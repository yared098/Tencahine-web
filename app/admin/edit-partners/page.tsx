...
export default function EditPartners() {
  ...
  const addHospital = () => {
    const newHosp = { name: "Partner Name", image: "https://via.placeholder.com/200x200?text=Logo" };
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] pb-44 text-slate-900">
      {/* Soft Nav - No Borders */}
      <nav className="sticky top-0 z-50 bg-white/60 backdrop-blur-xl px-6 py-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            {/* <button onClick={() => router.back()} className="group flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors">
              <i className="bi bi-arrow-left text-lg"></i> Back
            </button> */}
      ...
      <form onSubmit={handleSave} className="max-w-7xl mx-auto p-6 space-y-20">
        ...
        <div className="bg-white p-8 md:p-12 rounded-[3rem] shadow-sm">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-1">
              <h2 className="text-2xl font-black italic mb-2 tracking-tight">Messaging</h2>
              <p className="text-slate-400 text-sm font-medium">Define the core narrative of your partnerships.</p>
            </div>
            <div className="lg:col-span-2 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-blue-500 tracking-widest ml-1">Main Header</label>
                  <input type="text" value={data.title} onChange={(e) => handleChange("title", e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold focus:ring-2 focus:ring-blue-100 transition-all outline-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-blue-500 tracking-widest ml-1">Grid Heading</label>
                  <input type="text" value={data.hospitalSectionTitle} onChange={(e) => handleChange("hospitalSectionTitle", e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold focus:ring-2 focus:ring-blue-100 transition-all outline-none" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-300 tracking-widest ml-1">Detailed Subtitle</label>
                <textarea rows={2} value={data.subtitle} onChange={(e) => handleChange("subtitle", e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl p-4 font-medium text-slate-500 focus:ring-2 focus:ring-blue-100 transition-all outline-none resize-none" />
              </div>
            </div>
          </div>
        </div>

        {/* SECTION 2: PILLARS - No borders, just soft depth */}
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {data.items?.map((item: any, idx: number) => (
              <div key={idx} className="group bg-white p-8 rounded-[2.5rem] shadow-md shadow-slate-100/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 flex flex-col relative">
        ...
        {/* SECTION 3: LOGOS - Now light background */}
            ...
            {data.hospitals?.map((hosp: any, idx: number) => (
              <div key={idx} className="group relative bg-white rounded-3xl p-6 shadow-sm hover:shadow-lg transition-all">
                ...
                <div className="aspect-square bg-slate-50/50 rounded-2xl mb-4 p-4 flex items-center justify-center overflow-hidden">
                  <img src={hosp.image} alt="Logo" className="max-h-full max-w-full object-contain mix-blend-multiply transition-transform group-hover:scale-110 duration-500" />
                </div>
        ...
        <div className="fixed bottom-12 left-0 right-0 z-50 px-6">
          <div className="max-w-md mx-auto">
            ...
            <button type="submit" disabled={saving} className="w-full py-6 bg-blue-600 text-white rounded-[2.5rem] font-black text-sm uppercase tracking-[0.2em] hover:bg-blue-500 transition-all shadow-[0_25px_60px_-12px_rgba(37,99,235,0.4)] flex items-center justify-center gap-3">
